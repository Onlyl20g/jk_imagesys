...
; (function ($) {
  $.fn.easyUpload = function (opts) {
    var defaults = {
      ...
      multiNum: 20,//多文件上传时允许的有效文件数
      showNote: true,//是否展示文件上传说明
      note: '提示：最多上传20个文件，超出默认前五个，支持格式为：doc、docx、pdf',//文件上传说明
      showPreview: true,
      url: '',//上传文件地址
    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += '<input type="file" capture="camera" accept="image/*" '
          $html += option.multi ? 'multiple ' : '';
          $html += 'class="fileInput1" data-count="0" style="display:none;" />';
          $html += '<span style="margin-right: 5px" class="easy_upload_select1 noselect"><i style="color: green" class="mdui-icon material-icons">&#xe439;</i> </span>';

          $html += '<input type="file"'
          $html += option.multi ? 'multiple ' : '';
          $html += 'class="fileInput" data-count="0" style="display:none;" />';
          $html += '<span class="easy_upload_select noselect mdui-color-blue" style="margin-right: 5px">' + option.selectText + '</span>';
          /*$html +='<i class="mdui-icon material-icons">&#xe3b0;</i>'*/

          $html += '<span style="margin-right: 5px" class="easy_upload_head_btn1 noselect mdui-color-blue">上传</span>';
          $html += '<span class="easy_upload_head_btn2 noselect mdui-color-blue">清空</span>';
         // $html += '<span style="width: 135px" class="easy_upload_head_btn3 selectFolder  mdui-color-blue" onclick="upFile(this);">文件夹选择</span>';
          //$html += '<span style="margin-left: 10px;"><input readonly="readonly" class="form-control " style="width: 100px;height: 30px;" type="text" id="folderName" /><input class="folderId" style="display: none;" type="text"  value="" /></span>';
          $html += '<i class="easyUploadIcon noselect head_check" style="visibility: hidden" data-checked="no">&#xe693;</i>';
        ...
        bindHead: function () {
          ...
          $('.easy_upload_select1').off('click').click(function () {
            $(this).parent().find('.fileInput1').trigger('click');
          });
          ...
          $('.fileInput1').off('change').change(function () {
             /*count = Number($(this).attr('data-count'));*/
            var fileArr = [];
            var files = this.files;
            for (var i = 0; i < files.length; i++) {
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            if (fileType == 'bmp' || fileType == 'jpg' || fileType == 'jpeg' || fileType == 'png' || fileType == 'gif') {
              ...
              preview = '<img class="easy_upload_img" style="width: 100%;height: 100%" onclick="lookImg(this.src)" src="' + imgSrc + '" />';
            } else if (fileType == 'rar' || fileType == 'zip' || fileType == 'arj' || fileType == 'z') {
              preview = '<i style="width: 100px;height: 80px;" class="easy_upload_icon easyUploadIcon">&#xe69d;</i>';
            } else {
              preview = '<i style="width: 100px;height: 80px;" class="easy_upload_icon easyUploadIcon">&#xe64d;</i>';
            }
            var sHtml = '';
            sHtml += '<b class="status status1" style="color: green">可以上传</b>';
            sHtml += '<b class="status status2">等待上传</b>';
            sHtml += '<b class="status status3">上传中</b>';
            sHtml += '<b class="status status4">上传失败</b>';
            sHtml += '<b class="status status5">上传成功</b>';
            var $html = '';
            $html += '<li class="easy_upload_queue_item" data-index="' + file.index +'">';
            $html += option.showPreview ? '<div style="width: 100px;height: 90px;overflow-y: hidden" class="easy_upload_preview queue_item-section">' + preview + '</div>' : '';
            $html += '<div class="easy_upload_file1 queue_item-section">';
            $html += '<p class="easy_upload_filename clean_class phoneCss">'+ f.name +'</p>';
            $html += '<p style="visibility:hidden;" class="easy_upload_progress phoneCss">';
            $html += '<span class="easy_upload_bar"></span>';
            $html += '</p>';
            $html += '</div>';
            $html += '<div class="easy_upload_file2 queue_item-section">';
            $html += '<p class="easy_upload_fiesize clean_class">' + F.formatFileSize(f.size) +'</p>';
            $html += '<p class="easy_upload_percent" style="visibility:hidden;">0%</p>';
            $html += '</div>';
            $html += '<div class="easy_upload_status queue_item-section" style="margin-left: 110px;position: absolute">';
            $html += file.allow ? sHtml : '<b class="status status6">文件过大</b>';
            $html += '</div>';
            /*$html += '<div class="easy_upload_status queue_item-section">';
            //$html += '<input class="folderId" style="display: none;" type="text"  value="" />';
            $html += '</div>';*/
            //$html += '<div class="easy_upload_status queue_item-section">';
            //$html += '<span style="margin-left: 10px;"><input readonly="readonly" class="layui-layer-wrap" style="width: 120px;height: 30px;" type="text" id="folderName" value="'+sessionStorage.getItem("useFoledePaths")+'" /><input class="folderId" style="display: none" type="text"  value="'+sessionStorage.getItem("useFolders")+'" /></span>';
            /*$html += '&nbsp;<p class="easy_upload btn selectFolder  mdui-color-blue" onclick="upFile(this);" style="width: 100px;">文件夹选择</p>';*/
            //$html += '</div>';
						//$html += '<div class="queue_item-section"><select  class="fileLabel"  id="a'+selectId+'" name="fileLabel" style="width:100px;" onmousedown="if(this.options.length>3){this.size=4}" onblur="this.size=1" onchange="this.size=1">';
                       /* $html += '<div class="queue_item-section"><select  class="fileLabel" name="fileLabel" style="width:100px;">';
                        $html += LabelHtml;
						$html += '</select></div>';*/
            $html += '<div class="easy_upload_btn queue_item-section" style="position:absolute;margin-left: 88px;margin-top: -34px">';
            //$html += file.allow ? '<p class="easy_upload_upbtn btn noselect mdui-color-blue">上传</p>' : '';
           /* $html += '<p class="easy_upload_delbtn btn noselect mdui-color-orange" style="margin-left: -50px">删除</p>';*/
            $html += '<i style="color: red" class="mdui-icon material-icons easy_upload_delbtn  noselect">&#xe14c;</i>'
            $html += '</div>';
            $html += '<div class="easy_upload_checkone queue_item-section" style="visibility: hidden">';
            $html += '<i class="easyUploadIcon noselect queue_check queue_check_allow_'+ file.allow +'" data-checked="yes" data-up="1">&#xe693;</i>';
            $html += '</div>';
            if (option.multi) {
              $(queueUl).append($html);
            } else {
